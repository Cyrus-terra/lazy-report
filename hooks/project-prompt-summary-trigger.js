...
(async () => {
  try {
    ...
    const additionalContext = isDailyReport ? [
      ...
      `- 当前项目根目录：${root}`,
      `- 项目名称：${project}`,
      '- 读取当前项目根目录下的 CLAUDE_CODE_CHANGES_*.md 作为事实来源。',
      ...
      '- 如果项目根目录不存在 CLAUDE_CODE_CHANGES_*.md，说明改动记录只会在 Lazy Report 插件已安装启用、hooks 已加载，并且 Claude 在该项目执行过 Write/Edit/NotebookEdit 后生成。请提示用户检查插件是否启用、/hooks 中是否能看到 Lazy Report hooks，并打开 /hooks 或重启 Claude Code。',
    ].join('\n') : [
